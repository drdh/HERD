...
class TransformerModel(nn.Module):
    ...
    def __init__(
        ...
        self.decoder = make_mlp_default([self.decoder_input_dim] + [64] + [output_size],
                final_nonlinearity=final_nonlinearity, nonlinearity=nonlinearity) 

        self.init_weights(zero_initialize_final_layer)
    ...
    def init_weights(self, zero_initialize_final_layer):
        initrange = 0.1
        self.encoder.weight.data.uniform_(-initrange, initrange)
        self.decoder[2].bias.data.zero_()
        if zero_initialize_final_layer: 
            # self.decoder[2].weight.data.zero_()
            initrange = 0.01
        ...
        if self.use_transformer:
            ...
            if return_attention:
                obs_embed_t, attention_maps = self.transformer_encoder.get_attention_maps(
                    ...
                    design_cluster = design_cluster, attn_aggregation = attn_aggregation,)
            else:
