...
def run_ga(experiment_name, structure_shape, pop_size, max_evaluations, train_iters, num_cores, seed, env_name):
    print()

    experiment_name = f"{experiment_name}_{env_name}_seed_{seed}"
    ...
    if not is_continuing: 
        for i in range (pop_size):
            ...
            # a = np.full((5,5), 4.0)
            # temp_structure = (a, get_full_connectivity(a))
    ...
    #read status from file
        ...
        for g in range(start_gen+1):
            for i in range(pop_size):
                save_path_structure = os.path.join(home_path, "generation_" + str(g), "structure", str(i) + ".npz")
    ...
    while True:
        ...
        save_path_structure = os.path.join(home_path, "generation_" + str(generation), "structure")
        save_path_controller = os.path.join(home_path, "generation_" + str(generation), "controller")
        ...
        for structure in structures:
            ...
            if structure.is_survivor:
                save_path_controller_part = os.path.join(home_path, "generation_" + str(generation), "controller",
                    "robot_" + str(structure.label) + "_controller" + ".pt")
                save_path_controller_part_old = os.path.join(home_path, "generation_" + str(generation-1), "controller",
        ...
        temp_path = os.path.join(home_path, "generation_" + str(generation), "output.txt")
        f = open(temp_path, "w")
